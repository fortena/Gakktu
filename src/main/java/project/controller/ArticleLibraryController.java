...
import project.service.article/ArticleLibraryService;
import project.persistence.repositories.article/ArticleLibraryRepository;
...
public class ArticleLibraryController {
    ...
    @Autowired
    public ArticleLibraryController(ArticleService articleSerivce) {
    ...
    public String rosterViewGet(Model model){
        ...
        model.addAttribute("articles", articleSerivce.findAll());


        // Return the view (not sure here)
        return "articles/ArticleLibrary";
    }
    ...
    public String rosterViewPost(@ModelAttribute("article") Article article,
                                 Model model){

        //Check the availability of the User name
        String newName=article.getTitle();
        Boolean regStatus;

        if (articleSerivce.findByTitle(newName).size()==0) {
            // Save the User that we received from the form
            articleSerivce.save(article);
            regStatus = true;
        }
        else{regStatus=false;}

        // Here we get all the Postit Notes (in a reverse order) and add them to the model
        model.addAttribute("aticles", articleSerivce.findAll());
    ...
    public String registerGetUserFromName(@PathVariable String name,
        ...
        model.addAttribute("users", articleSerivce.findByName(name));
